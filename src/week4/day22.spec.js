...
import { executeFullRebootSteps, executeRebootSteps, intersect, parseRebootSteps, cubeValue, isStepInside } from './day22.js'
...
describe('Day 22: Reactor Reboot', () => {
  ...
  describe('Part 1: how many cubes are on?', () => {
    ...
    it('keeps step inside -50,50', () => {
      const steps = parseRebootSteps(example)

      expect(isStepInside(steps[0])).to.equal(true)
      expect(isStepInside(steps[steps.length-1])).to.equal(false)
    })
